...
// PB-MPM solver mechanics (Lewin 2024). The constraint projection + integration constitutive
// code lives in mpm_material.cpp; this file holds the grid transfers, collider, sleeping, and
// queries. Everything is in DISPLACEMENT (= velocity·dt); a timestep iterates the inner loop
...
// P2G: scatter mass + displacement-momentum mass·(d + D·dpos). No stress (the constraint
// solve replaced it). Asleep particles still scatter (d = 0, D frozen) so support is preserved.
void MpmSim::_p2g() {
...
// Mass-weight the grid momentum to a displacement, resolve collider contact, clamp to domain.
void MpmSim::_grid_update() {
	for (int iz = 0; iz < _dim; iz++) {
		for (int iy = 0; iy < _dim; iy++) {
			for (int ix = 0; ix < _dim; ix++) {
				...
				if ((iy < 2 && disp.y < 0.0) || (iy >= _dim - 2 && disp.y > 0.0)) {
					disp.y = 0.0;
				}
...
// G2P: gather grid displacement → particle displacement d, and rebuild the APIC affine D.
...
bool MpmSim::is_finite() const {
	for (uint32_t i = 0; i < _x.size(); i++) {
		if (!_x[i].is_finite() || !_d[i].is_finite()) {
			return false;
		}
	}
	return true;
